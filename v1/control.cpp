...
class Attractor : public Processor {
	...
	public:
		...
		void runProcesses(const Uint8* keystate, float timeInterval) override {
			...
			float radius = totalMass * 100;

			Vector2 attractorPosition = Vector2{ 300, 300 };
			...
			for (int j = 0; j < objectCount; j++) {
				...
				Vector2 attractorCenter = attractorPosition.translate(negativePosition);
				Vector2 forceDirection = attractorCenter.normalize();
				float mass = (*attracteePointer).motion.mass;

				if (attractorCenter.magnitude() < radius) {
				...
				float factor = (attractorCenter.magnitude() / radius);
				Vector2 newForce = processedForce.translate(forceDirection.normalize().scalarTransform(factor * mass * totalMass / timeInterval));
